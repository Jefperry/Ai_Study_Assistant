...
class Summary(db.Model):
    ...
    title = db.Column(db.String(200), nullable=False, default="Untitled Summary")
    ...
    # Metadata
    word_count = db.Column(db.Integer, default=0)  # Word count of original content
    processing_time = db.Column(db.Float, default=0.0)  # Time taken to process (seconds)
    ...
    def to_preview_dict(self):
        ...
        summary_preview = self.summary[:100] + "..." if self.summary and len(self.summary) > 100 else self.summary
        return {
            "id": self.id,
            "title": self.title,
